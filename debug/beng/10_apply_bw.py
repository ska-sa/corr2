...
def get_snap_data(offset_words):
    ...
    """
    width_bytes0 = f.snapshots.bfsnap_bw_ss.width_bits / 8
    width_bytes1 = f.snapshots.bfsnap_bwtime_ss.width_bits / 8
    ...
    f.snapshots.bfsnap_bw_ss.arm(offset=offset_bytes0)
    f.snapshots.bfsnap_bwtime_ss.arm(offset=offset_bytes1)
    f.registers.bf_config.write(snap_arm=1)
    d = f.snapshots.bfsnap_bw_ss.read(arm=False)['data']
    d2 = f.snapshots.bfsnap_bwtime_ss.read(arm=False)['data']
    d.update(d2)
...
if d['bwsync'][7] != 1:
    for ctr in range(4096):
        print(ctr, end='')
        for key in d.keys():
            print('%s(%i)' % (key, d[key][ctr]), end='')
        print('')
    print('Sync was not found where we expect it.')
    raise RuntimeError
if d['intime27'][8] != d['intime27'][7] + 1:
    print('Time jumped from %i to %i?' % (d['intime27'][7], d['intime27'][8]))
    raise RuntimeError
snaplen = len(d['bwd0'])
...
while True:
    ...
    for dctr, dword in enumerate(d['bwd0']):
        ...
        if freq != lastfreq:
            if lastfreq != -1:
                dstr += '%i' % freqctr
                if (freqctr != 1024) and (not first_freq):
