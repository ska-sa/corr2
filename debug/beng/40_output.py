...
# for bitstream = /home/paulp/r2_b4a4x256f_4gbe_2016_Feb_17_1624.fpg
...
def get_snap_data(offset_words, man_valid=False):
    ...
    """
    width_bytes0 = f.snapshots.bfsnap_out_ss.width_bits / 8
    ...
    f.snapshots.bfsnap_out_ss.arm(offset=offset_bytes0, man_valid=man_valid)
    f.registers.bf_config.write(snap_arm=1)
    d = f.snapshots.bfsnap_out_ss.read(arm=False)['data']
    print('Read %i words' % len(d[d.keys()[0]])
    return d
...
while True:
    ...
    freqctr = 0
    first_freq = True
    timestamp = d['time27'][0]
    # process it
    for dctr, dword in enumerate(d['d32']):
        # take only pol0
        dword >>= 16
        freq = convert_to_freq(dword, 1)
        dstr = '%i %i %i %i %i %i %s' % (dctr, d['time27'][dctr],
                                         d['sync'][dctr], d['en'][dctr],
                                         dword, freq,
                                         'ERROR' if freq >= (FIRST_FREQ + 256) else '')
        ...
        if d['time27'][dctr] != timestamp:
            raise RuntimeError('The time changed in the packet?')
        if freq != lastfreq:
            if lastfreq != (FIRST_FREQ - 1):
                dstr += '%i' % freqctr
