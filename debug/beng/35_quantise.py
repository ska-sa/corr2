...
def get_snap_data(offset_words, man_valid=False):
    ...
    """
    width_bytes0 = f.snapshots.bfsnap_quant_ss.width_bits / 8
    ...
    f.snapshots.bfsnap_quant_ss.arm(offset=offset_bytes0, man_valid=man_valid)
    f.registers.bf_config.write(snap_arm=1)
    d = f.snapshots.bfsnap_quant_ss.read(arm=False)['data']
...
while True:
    ...
    # process it
    for dctr, dword in enumerate(d['qt_d32']):
        # take only pol0
        dword >>= 16
