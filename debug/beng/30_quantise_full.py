...
def convert_to_freq(num, beamweight=1.0):
    ...
    try:
        a = num
        p1r = bin2fp(a >> 44, 44, 24, True) * (1.0 / beamweight)
        p1i = bin2fp(a & ((2**44)-1), 44, 24, True) * (1.0 / beamweight)
...
def get_snap_data(offset_words, man_valid=False):
    ...
    """
    width_bytes0 = f.snapshots.bfsnap_qfull_ss.width_bits / 8
    offset_bytes0 = offset_words * width_bytes0
    print('Reading snapshot 0 at offset %i' % offset_bytes0
    f.registers.bf_config.write(snap_arm=0)
    f.snapshots.bfsnap_qfull_ss.arm(offset=offset_bytes0, man_valid=man_valid)
    f.registers.bf_config.write(snap_arm=1)
    d = f.snapshots.bfsnap_qfull_ss.read(arm=False)['data']
    print('Read %i words' % len(d.keys()[0])
    return d
...
while True:
    ...
    freqctr = 0
    first_freq = True
    timestamp = d['qt_time27'][0]
    # process it
    for dctr, dword in enumerate(d['qt_d88']):
        freq = convert_to_freq(dword, 1)
        dstr = '%i %i %i %i %i %i %s' % (dctr, d['qt_time27'][dctr],
                                         d['qt_sync'][dctr], d['qt_en'][dctr],
                                         dword, freq,
