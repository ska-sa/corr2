...
xeng_fpga = CasperFpga(args.host)
...
def get_snapdata():
    xeng_fpga.registers.snapctrl.write(arm=False)
    xeng_fpga.snapshots.xeng_snap0.arm()
    xeng_fpga.snapshots.xeng_snap1.arm()
    xeng_fpga.registers.snapctrl.write(arm=True)
    d = xeng_fpga.snapshots.xeng_snap0.read(arm=False)['data']
    d1 = xeng_fpga.snapshots.xeng_snap1.read(arm=False)['data']
    xeng_fpga.registers.snapctrl.write(arm=False)
    d.update(d1)
    return d
...
def print_snapshot(snapdata):
    ...
    for ctr in range(0, snaplen):
        if args.showall or snapdata['eof'][ctr] == 1:
            print('%5d:' % ctr, end='')
            for key in snapkeys:
                print('%s(%d)  ' % (key, snapdata[key][ctr]), end='')
            print('')
