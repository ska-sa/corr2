...
snapshot_missing = []
for fpga_ in fpgas:
    if not 'p0_data_ss' in fpga_.snapshots.names():
...
# arm the snaps
fpgautils.threaded_fpga_operation(fpgas, 10, lambda fpga_: fpga_.snapshots.p0_data_ss.arm())
fpgautils.threaded_fpga_operation(fpgas, 10, lambda fpga_: fpga_.snapshots.p1_data_ss.arm())
...
# read the data
snapdata_p0 = fpgautils.threaded_fpga_operation(fpgas, 10, lambda fpga_: fpga_.snapshots.p0_data_ss.read(arm=False))
snapdata_p1 = fpgautils.threaded_fpga_operation(fpgas, 10, lambda fpga_: fpga_.snapshots.p1_data_ss.read(arm=False))
...
if args.checktvg:
    ...
    for fpga, fpga_data in unpacked_data.items():
        ...
        for pol_data in [(fpga_data['p0'], 0), (fpga_data['p1'], 1)]:
            ...
            fftdata = numpy.abs(numpy.fft.fft(allsamples))
