...
class CorrRx(threading.Thread):
    def __init__(self, mode='cont', port=7148, log_handler=None, log_level=logging.INFO,
                 spead_log_level=logging.DEBUG, **kwargs):
        # if log_handler == None:
        ...
        for heap in spead.iterheaps(rx):
            ...
            for name in ig.keys():
                ...
                if not item._changed and (name in datasets.keys()):
                    continue  # the item is not marked as changed, and we have a record for it
                ...
                if name not in datasets.keys():
                 # check to see if we have encountered this type before
                    ...
                    f.create_dataset(name, [1] + ([] if list(shape) == [1] else list(shape)),
                ...
                if name.startswith("xeng_raw"):
                    ...
                    ig_sd.add_item(name='sd_timestamp',
                                   id=0x3502,
                                   description='Timestamp of this sd frame in '
                                               'centiseconds since epoch (40 bit limitation).',
    ...
    def rx_inter(self, data_port=7148, sd_ip='127.0.0.1', sd_port=7149, acc_scale=True, filename=None, **kwargs):
        ...
        for heap in spead.iterheaps(rx):
            ...
            for name in ig.keys():
                ...
                if not item._changed and (name in datasets.has_keys()):
                    continue
                ...
                if name in meta_required:
                    ...
                    if len(meta_required) == 0:
                        sd_frame = np.zeros((meta['n_chans'], meta['n_bls'], 2), dtype=np.float32)
                        logger.info("Got all required metadata. Initialised sd frame to shape %s" %
                                    (str(sd_frame.shape)))
                        meta_required = ['n_chans', 'bandwidth', 'n_bls', 'n_xengs', 'center_freq', 'bls_ordering']
                        ig_sd = spead.ItemGroup()
                        for meta_item in meta_required:
                ...
                if sd_frame is not None and name.startswith("timestamp"):
                    ...
                    timestamp = ig['sync_time'] + (ig[name] / ig['scale_factor_timestamp'])
                    local_time = time.time()
                    print "Decoded timestamp for Xeng", xeng_id, ":", timestamp, " (", time.ctime(timestamp), \
                        ") @ %.4f" % local_time, " ", time.ctime(local_time), "diff(", local_time-timestamp, ")"

                    # is this timestamp in the past?
                    if current_timestamp > timestamp:
                        error_string = "Timestamp %.2f (%s) is earlier than the current timestamp %.2f (%s). " \
                                       "Ignoring..." % (timestamp, time.ctime(timestamp),
                                                        current_timestamp, time.ctime(current_timestamp))
                        logger.warning(error_string)
                        continue

                    # is this a new timestamp before a complete set?
                    if (timestamp > current_timestamp) and sd_slots.any():
                        error_string = "New timestamp %.2f from Xeng%i before previous set %.2f sent" % \
                                       (timestamp, xeng_id, current_timestamp)
                        logger.warning(error_string)
                        sd_slots = np.zeros(meta['n_xengs'])
                    ...
                    if timestamp <= sd_slots[xeng_id]:
                        error_string = 'Xeng%i already on timestamp %.2f but got %.2f now, THIS SHOULD NOT HAPPEN' %\
                                       (xeng_id, sd_slots[xeng_id], timestamp)
                ...
                if timestamp is not None and sd_frame is not None and sd_slots is not None and sd_slots.all():
                    ...
                    ig_sd.add_item(name='sd_data', id=0x3501,
                                   description="Combined raw data from all x engines.",
                                   ndarray=(sd_frame.dtype, sd_frame.shape))
                    ig_sd.add_item(name='sd_timestamp', id=0x3502,
                    ...
                    scale_factor = (meta['n_accs'] if 'n_accs' in meta.keys() else 1)
                    logger.info("Sending signal display frame with timestamp %i (%s). %s. @ %.4f" %
                                (timestamp, time.ctime(timestamp),
                                 "Unscaled" if not acc_scale else "Scaled by %i" % scale_factor, time.time()))
                    ig_sd['sd_data'] = sd_frame.astype(np.float32) if not acc_scale else \
                        (sd_frame / float(scale_factor)).astype(np.float32)
...
if __name__ == '__main__':
    ...
    p.add_option(
        ...
        action='store_false',
        default=True,
        help='Do not autoscale the data by dividing down by the number of accumulations. '
             'Default: Scale back by n_accs.',)
    p.add_option(
        '-v',
        '--verbose',
        dest='verbose',
...
sd_port = 7149          # config['sig_disp_port']
mode = 'cont'           # config['xeng_format']
