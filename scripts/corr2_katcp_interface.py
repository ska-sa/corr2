...
import sys, time, logging, Queue, argparse

import corr2, katcp, katcp.kattypes as kattypes
...
class Corr2_server(katcp.DeviceServer):
    '''A server that creates a corr2-based instrument.
    '''
    ## interface version information.
    VERSION_INFO = ('Python CASPER packetised correlator server', 0, 1)
    ## device server build / instance information.
    BUILD_INFO = ('corr2', 0, 1, 'rc1')

    def __init__(self, *args, **kwargs):
        super(Corr2_server, self).__init__(*args, **kwargs)
    ...
    @kattypes.request(kattypes.Str(default='/dev/null'), kattypes.Int(default=100), include_msg=True)
    @kattypes.return_reply()
    def request_connect(self, sock, orgmsg, config_file, log_len):
        '''Connect to all the ROACH boards.
        Please specify the config file and the log length.
        Clears any existing log.
        Call this again if you make external changes to the config file to reload it.
        '''
        self.loghandler = None #= corr2.fxcorrelator.Fx_correlator.log_handlers.DebugLogHandler(log_len)
        try:
            self.instrument = corr2.fxcorrelator.Fx_correlator(name='fxcorr_%i' % int(time.time()), description='rtscorr2', config_file=config_file)
            self.instrument.setup_logging(self.loghandler, logging.INFO)
        except Exception as err_msg:
            return ('fail', err_msg)
        return ('ok', )

    @kattypes.request(kattypes.Int(default=100))
    @kattypes.return_reply()
    def request_initialise(self, sock, n_retries):
        '''Initialise the instrument.
        This programs the FPGAs, configures network interfaces etc.
        Includes error checks.
        Consult the log in event of errors.
        '''
        if self.instrument is None:
            return ('fail', '... you haven''t connected yet!')
        try:
            self.instrument.initialise(n_retries)
        except:
            return ('fail', 'Correlator could not initialise. Check the log.')
        return ('ok', )

#    @kattypes.request(include_msg=True)
#    @kattypes.return_reply(kattypes.Int(min=0))
...
#        return ('ok', )
#
#    @kattypes.request(kattypes.Int(), kattypes.Str())
#    @kattypes.return_reply(kattypes.Str())
...
#
#    @kattypes.return_reply(kattypes.Str(), kattypes.Str())
...
#        Returns health tree informs for each engine in the system.
#        '''
#        if self.c is None:
...
#        '''Rearms the system.
#        Returns the time at which the system was synchronised in ms since unix epoch.
#        '''
#        if self.c is None:
#            return ('fail', '... you haven''t connected yet!')
#        try:
...
#
#    @kattypes.request(kattypes.Str(), include_msg=True)
...
#
#    @kattypes.request(kattypes.Str(), kattypes.Int(default=1), include_msg=True)
...
#
#    @kattypes.request(kattypes.Float(min=0))
#    @kattypes.return_reply(kattypes.Float())
...
#        '''Set the fringe rate and delay compensation config for a given antenna.
#        Parms: antpol, fringe_offset (degrees), fr_rate (Hz), delay (seconds), delay rate, load_time (Unix seconds), <ignore check>.
#        If there is a seventh argument, don't do any checks to see if things loaded properly.
#        If the load time is negative, load asap.
#        '''
#        if self.c is None:
#            return katcp.Message.reply(orgmsg.name, 'fail', '... you haven''t connected yet!')
#        ant_str=orgmsg.arguments[0]
...
#
#    @kattypes.request(kattypes.Str(), kattypes.Str(), include_msg=True)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Send commands to a corr2 system.',
                        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('-l', '--listen', dest='HOSTNAME', type=str, action='store',
                        default='all', help='listen to this host')
    ...
    print 'Corr2_server listening on port %d, ctrl-C to terminate server' % args.PORT
    restart_queue = Queue.Queue()
    server = Corr2_server(args.HOSTNAME, args.PORT)
