    ...
    def start_stop(stopstart):
        ...
        if fpgas[0].registers.names().count('control') > 0:
            if fpgas[0].registers.control.field_names().count('gbe_out_en') > 0:
                ...
                fpgautils.threaded_fpga_operation(fpgas, lambda fpga: fpga.registers.control.write(gbe_out_en=stopstart))
            elif fpgas[0].registers.control.field_names().count('gbe_txen') > 0:
                print '\tSim f-engine control registers found.'
                fpgautils.threaded_fpga_operation(fpgas, lambda fpga: fpga.registers.control.write(gbe_txen=stopstart))
            elif fpgas[0].registers.control.field_names().count('comms_en') > 0:
                print '\tOld-style fengine control registers found.'
                fpgautils.threaded_fpga_operation(fpgas, lambda fpga: fpga.registers.control.write(comms_en=stopstart))
            ...
            if fpgas[0].registers.ctrl.field_names().count('comms_en') > 0:
                ...
                print fpgautils.threaded_fpga_operation(fpgas, lambda fpga: fpga.registers.ctrl.write(comms_en=stopstart))
        else:
