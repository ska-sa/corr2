    ...
    def __init__(self, parent, name, mac=None, ip_address=None, port=None, info=None):
        ...
        if info != None:
            ...
            if fabric_ip.find('(2^24) + ') != -1:
                info['fab_ip'] = fabric_ip.replace('*(2^24) + ', '.').replace('*(2^16) + ', '.').replace('*(2^8) + ', '.').replace('*(2^0)', '')
            fabric_mac = info['fab_mac']
            if fabric_mac.find('hex2dec') != -1:
                fabric_mac = fabric_mac.replace('hex2dec(\'', '').replace('\')', '')
                info['fab_mac'] = fabric_mac[0:2] + ':' + fabric_mac[2:4] + ':' + fabric_mac[4:6] + ':' + fabric_mac[6:8] + ':' + fabric_mac[8:10] + ':' + fabric_mac[10:]
            mac = info['fab_mac']
            ip_address = info['fab_ip']
            port = info['fab_udp']
        if mac == None or ip_address == None or port == None:
