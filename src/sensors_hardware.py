...
def setup_sensors(sensor_manager):
    """
    Hardware sensors to be reported to CAM
    ...
    host_executors = {
        host.host: ThreadPoolExecutor(max_workers=1) for host in all_hosts
    }
    general_executor = ThreadPoolExecutor(max_workers=1)
    if not sens_man.instrument.initialised():
        raise RuntimeError('Cannot set up hardware sensors until instrument is '
                           'initialised.')
    ...
    _setup_sensors(sens_man, general_executor, host_executors, ioloop)
